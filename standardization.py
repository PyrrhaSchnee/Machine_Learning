...
For the mixed ones : Empowered vs Prescience
For the separated ones : Push vs Midi-chlorien

One additional step : standardization
which means make the centre of the data points around 0
and make its standard deviation = 1;
Values are not necessarily between 0 and 1 (can still be large / small)
z = (x - mean_x) / std
"""
...
def standardize(data: pd.DataFrame, features: list[str]) -> pd.DataFrame:
    """
    ...
    If std is 0 or infinite, standardized values are 0 for
    those who are not NaN
    """
    ...
    while i < len(features):
        ...
        mean_x: float = float(np.nanmean(x))
        std: float = float(np.nanstd(x, ddof=0))
...
def pretty_print(
    ...
    """
    Pretty print the standardized values
    """
    print(
        f"\n----------BEGINNING OF OUTPUT------------Standardized values of the file {filename}:"
    )
    ...
    while i < values.shape[0]:
        ...
        i += 1
    print(
        f"----------END OF OUTPUT------------Standardized values of the file {filename}\n"
    )
...
def main() -> int:
    ...
    try:
        ...
        data_train_std = standardize(data_train, features_train)
        data_test_std = standardize(data_test, features_test)
